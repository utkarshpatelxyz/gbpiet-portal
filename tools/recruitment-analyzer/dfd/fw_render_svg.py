...
def draw(n):
    ...
    if t=="title":
        ...
        g.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" rx="9" fill="{C["az"]}"/>')
        g.append(f'<circle cx="{x+22}" cy="{y+h/2}" r="13" fill="#fff"/>')
        g.append(f'<text x="{x+22}" y="{y+h/2+5}" font-size="14" font-weight="800" fill="{C["az"]}" text-anchor="middle">{esc(n.get("num",""))}</text>')
        g.append(text_block(x+40,y,w-46,h,n["label"],13,"#ffffff","800"))
    elif t=="term":
        r=h/2+5; pill_x=x+r
